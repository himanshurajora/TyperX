...
import { tokenize, languages as prismLanguages, Token } from 'prismjs'
...
function App() {
  ...
  const [tokens, setTokens] = useState<(string | Token)[]>([])
  ...
  const setInit = () => {
    ...
    if (currentLanguage && currentProgram) {
      ...
      // })
      setCode(
        (Data as any)['Programs'][currentPath][currentLanguage][currentProgram][
          'code'
        ]
      )
      setDescription(
      ...
      setTokens(
        tokenize(
          (Data as any)['Programs'][currentPath][currentLanguage][
            currentProgram
          ]['code'],
          prismLanguages['javascript']
        )
      )
    }
      ...
      tokens.forEach((token) => {
        if (typeof token === 'string') {
          token.split('').forEach((char) =>
            chars.push({
              char,
              color: TokenColors.common,
            })
          )
        } else {
          ;(token as Token).content
            .toString()
            .split('')
            .forEach((char) => {
              chars.push({
                char,
                color:
                  TokenColors[token.type as keyof typeof TokenColors] || 'pink',
              })
            })
        }
      })
  ...
  const loadNextProgram = () => {
    setCurrentProgram(
      Data[currentPath]['programlist'][
        Data[currentPath]['programlist'].indexOf(currentProgram) + 1 ===
        Data[currentPath]['programlist'].length
          ? 0
          : Data[currentPath]['programlist'].indexOf(currentProgram) + 1
      ]
    )
                ...
                <span
                  ...
                  className={`subtitle is-5 ${
                    codeBlock.status == 'C'
                      ...
                      : codeBlock.status == 'R'
                      ? 'current has-background-warning has-text-white'
                      : 'has-background-white has-text-black'
                  }`}
                  style={{ color: charColors?.[index]?.color }}
                >
                  {' '}
                  <br />{' '}
                </span>
                ...
                <span
                  ...
                  className={`subtitle is-5 ${
                    codeBlock.status == 'C'
                      ...
                      : codeBlock.status == 'R'
                      ? 'current has-background-warning has-text-white'
                      : 'has-background-white ' + charColors[index].color
                  }`}
                >
                  {codeBlock.character}
                </span>
